...
public abstract class FilteringVisitorSupport {
	...
	public static String stringify(final Artifact artifact) {
		if (artifact.getVersion() == null) {
			throw new RuntimeException("Artifact version and version range is null: " + artifact);
		}
		String uniqArtifactId = artifact.getGroupId() + ":" + artifact.getArtifactId() + ":" + artifact.getVersion();
	...
	protected final String stringifyNoVersion(final Artifact artifact) {
		...
		String uniqArtifactIdVersionLess = artifact.getGroupId() + ":" + artifact.getArtifactId();
	...
	protected final boolean isInScope(final DependencyNode node) {
		...
		if (scope == null) {
			log.debug("Null Scope For artifact: " + node.getArtifact().getArtifactId());
			return true;
		}
